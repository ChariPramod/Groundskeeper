...
Each explicit invocation is a new execution attempt with a new report ID. Replaying the exact same report through the storage function is idempotent, while conflicting reuse of its ID is rejected. If database persistence fails after execution, the local artifact is retained; automated artifact import/recovery is not yet implemented. Nothing is written to GitHub. Exit codes match the local verifier: 0 for no observed failures (possibly empty/skipped work), 1 for failed examples, and 2 for infrastructure/input/persistence errors.
